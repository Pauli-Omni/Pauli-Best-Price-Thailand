...
(function (global) {
  ...
  function classify(utterance, _lang) {
    ...
    var di;
    for (di = 0; di < DELIVERY_INTENT_ORDER.length; di += 1) {
      if (PATTERNS[DELIVERY_INTENT_ORDER[di][0]].test(text)) {
        return localHit(DELIVERY_INTENT_ORDER[di][1]);
      }
    }
